...
export async function analyzeReviewImages(imageBuffers: Buffer[], mediaType: string, preferences: string[] = []): Promise<VisionResult> {
  ...
  out.redactionBoxes = (out.redactionBoxes || [])
    .filter((b) => typeof b.x === "number" && typeof b.y === "number" && b.w > 0 && b.h > 0)
    .map((b) => ({ ...b, image: typeof b.image === "number" && b.image >= 0 && b.image < n ? b.image : 0 }));
