...
export const HASH_TARGETS = [
  { label: "direct", env: "NEON_CHECK_DIRECT_URL" },
  { label: "pooled", env: "NEON_CHECK_POOLED_URL" },
  { label: "forbidden", env: "NEON_CHECK_FORBIDDEN_URL" }, // production URL 을 **일시 주입**했을 때만 계산
] as const;
...
export function hashToolUsage(): string[] {
  return [
    ...
    "  PowerShell:",
    "    $env:NEON_CHECK_DIRECT_URL = (Read-Host 'direct URL')      # 명령줄 리터럴 금지",
    "    $env:NEON_CHECK_POOLED_URL = (Read-Host 'pooled URL')",
    "    node --import tsx/esm scripts/neonCheck/hashTool.ts",
    "  출력은 `direct#<64hex>` / `pooled#<64hex>` 형식이며 URL·host 원문은 출력하지 않는다.",
  ];
}
