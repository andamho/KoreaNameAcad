...
interface Comment {
  ...
  notifyContact?: string | null; notifyContactType?: string | null;
}
  ...
  async function submitComment() {
    ...
    try {
      const res = await fetch('/api/experience-comments', {
        ...
        body: JSON.stringify({ pageId: 'husband-luck', nickname: nickname.trim(), totalStrokes: null, content: commentText.trim(), isPrivate, notifyContact: wantsNotify ? notifyContact.trim() : null, notifyContactType: wantsNotify ? notifyContactType : null }),
      });
        ...
        <div className="max-w-2xl mx-auto px-5 space-y-8">
          ...
          <div className="rounded-3xl bg-slate-900 dark:bg-slate-800 overflow-hidden shadow-2xl">
            <div className="px-6 pt-6 pb-4 border-b border-white/10">
              <p className="text-lg font-bold text-white mb-1">남편복 | 아내복 AI 분석</p>
          ...
          <div ref={commentRef} className="space-y-5">
                ...
                {wantsNotify && (
                  <div className="flex items-center gap-2 pl-5">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground flex-shrink-0">
                      <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="notifyType-husband" checked={notifyContactType === 'sms'} onChange={() => setNotifyContactType('sms')} className="accent-[#18a999]" />문자</label>
                      <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="notifyType-husband" checked={notifyContactType === 'email'} onChange={() => setNotifyContactType('email')} className="accent-[#18a999]" />이메일</label>
                    </div>
            ...
            <div className="space-y-3">
              ...
              {comments.map(c => c.isPrivate && !isAdmin && !c.content ? (
