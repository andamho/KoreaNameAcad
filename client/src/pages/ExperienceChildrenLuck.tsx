  ...
  async function submitComment() {
    ...
    try {
      const res = await fetch('/api/experience-comments', {
        ...
        body: JSON.stringify({ pageId: 'children-luck', nickname: nickname.trim(), totalStrokes: null, content: commentText.trim(), isPrivate, notifyContact: wantsNotify ? notifyContact.trim() : null, notifyContactType: wantsNotify ? notifyContactType : null }),
      });
                ...
                {wantsNotify && (
                  <div className="flex items-center gap-2 pl-5">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground flex-shrink-0">
                      <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="notifyType-children" checked={notifyContactType === 'sms'} onChange={() => setNotifyContactType('sms')} className="accent-[#18a999]" />문자</label>
                      <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="notifyType-children" checked={notifyContactType === 'email'} onChange={() => setNotifyContactType('email')} className="accent-[#18a999]" />이메일</label>
                    </div>
