...
```
`CONFIRM_EXECUTE` 를 **설정하지 않은 상태**가 dry-run 입니다. 이미 설정돼 있다면 먼저 **제거**하세요.
> ⚠️ **빈 값으로 두면 거부됩니다.** `$env:CONFIRM_EXECUTE = ""` 는 "끈 것"이 아니라 **오류**로 처리됩니다
> (지웠다고 착각한 상태를 조용히 통과시키지 않기 위함). 반드시 `Remove-Item` 으로 변수를 제거하세요.
> 활성화는 **정확한 문자열 `true`** 만 인정합니다 — `TRUE`·`True`·`1`·`yes`·`on`·`true `(뒤 공백) 전부 거부됩니다.
```powershell
